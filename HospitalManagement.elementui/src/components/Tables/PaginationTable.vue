                ...
                <el-table :data="datalist" v-loading="listLoading" element-loading-text="加载数据中~~~" :stripe="true"
                    ...
                    <template v-for="(item, idx) in columnList" :key="idx">
                        ...
                        <el-table-column v-else-if="item.type == 4" v-bind="item" align="center" :width="item.width || 100"
                            :prop="item.prop || 'Id'" :label="item.title" fixed="right">
                            <template #default="{ row }">
                        ...
                        <el-table-column v-else-if="item.type == 14" v-bind="item" align="center"
                            :width="item.width || 'auto'" :prop="item.prop || 'Id'" :label="item.title">
                            <template #default="{ row }">
                                <slot :name="item.key" :row="row"></slot>
                            </template>
                        </el-table-column>
...
const fetchDataList = async (searchWhere = {}) => {
    ...
    try {
        ...
        const dataList = Items.map((item, index) => {
            // 确保 item 不为空，如果为空则使用空对象
            if (!item || typeof item !== 'object') {
                item = {};
            }
            
            // 先复制所有原始字段到 dataItem（确保数据不丢失）
            const dataItem = { ...item };
            
            // 然后根据 column 配置处理特定字段
            props.column.forEach((element) => {
                if (element.template) {
                    dataItem[element.key] = element.template(item, index);
                } else {
                    const value = GetObjectValue(item, element.key);
                    if (value !== undefined) {
                        dataItem[element.key] = value;
                    }
                }
                ...
                if (element.type == 12) { // IMAGES类型
                    if (element.template) {
                        ...
                        const imageValue = GetObjectValue(item, element.key);
                        if (imageValue) {
                            dataItem[element.key] = imageValue.split(',');
                        }
                    }
                }
            });
            ...
            // 确保 status 字段被提取到顶层（从 item 中）
            // 由于后端使用 UPPER_CAMEL_CASE，字段名可能是 Status
            // 优先提取 Status（大驼峰）
            if (item && (item.Status !== undefined && item.Status !== null)) {
                dataItem.Status = item.Status;
                dataItem.status = item.Status; // 同时设置小写版本以便兼容
            }
            // 如果 Status 不存在，尝试 status（小写）
            else if (item && (item.status !== undefined && item.status !== null)) {
                dataItem.status = item.status;
                dataItem.Status = item.status; // 同时设置大写版本
            }
            
            // 确保 dataItem 至少包含 Id 字段（如果 item 有的话）
