<template>
    <div>
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改角色与权限关联' : '添加角色与权限关联'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/role_permission/List" :column="columnList" :where="where">
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/role_permission/Get`, { Id: Id });
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/role_permission/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/role_permission/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/role_permission/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
