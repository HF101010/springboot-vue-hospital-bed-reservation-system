...
public class AppUserServiceImpl extends ServiceImpl<AppUserMapper, AppUser> implements AppUserService {
    ...
    public PagedResult<AppUserDto> List(AppUserPagedInput input) {
        ...
        // 按ID从大到小排序（如果表中没有creation_time字段）
        queryWrapper = queryWrapper.orderByDesc(AppUser::getId);
        // 构建一个分页查询的model
   ...
   /**
    ...
    public String SignIn(AppUserDto input) {
        ...
        if (items.stream().count() == 0) {
            throw new CustomException("请检查登录的账号或者密码是否正确!");
        }
    ...
    public void Export(@RequestParam String query, HttpServletResponse response) throws IOException {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (appUser.getBirth() != null) {
                // Birth 现在是 LocalDate，直接输出为字符串，例如：2024-01-13
                row.createCell(5).setCellValue(new HSSFRichTextString(appUser.getBirth().toString()));
            }
        }
